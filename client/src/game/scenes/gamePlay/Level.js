...
export class Level extends SuperContainer {
    ...
    set({bonusStep, currentStep = 0, nextStepWin} = {}) {
        ...
        this.setHeroToPlatform(currentStep);

        if (nextStepWin) {
            this.setNextStepWin({step: currentStep + 1, nextStepWin}, true);
        }
    }

    setNextStepWin({step, nextStepWin}, force) {
        return this.getPlatformByNumber(step).showWinValue(nextStepWin, force);
    }
