export const TaskAction = Object.freeze({
    PLAY_GAME: "play_game",
    INVITE_FRIEND: "invite_friend",        // Действие: пригласить друга
    INVITE_FRIEND_PREMIUM: "invite_friend_premium",        // Действие: пригласить друга
    CLAIM_DAILY_REWARD: "claim_daily",     // Действие: забрать ежедневную награду
    COLLECT_BONUS: "collect_bonus",        // Действие: собрать бонус
    SHARE_EMAIL: "share_email",            // Действие: поделиться email
    SHARE_PHONE: "share_phone",            // Действие: поделиться телефоном
    SHARE_X_ACCOUNT: "share_x_account"     // Действие: поделиться аккаунтом X
});
